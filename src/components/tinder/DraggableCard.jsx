...
class DraggableCard extends Component {
  ...
  calculatePosition(deltaX, deltaY) {
    const bsY = deltaY < 0 ? -1 : 1;
    const absY = Math.abs(deltaY);
    const reducedY = Math.sqrt(absY) * bsY;
  ...
  render() {
    const { x, y, animation, pristine, hide, className } = this.state;
    const style = translate3d(x, y);

    const classAnimate = animation ? 'animate' : pristine ? 'inactive' : '';
    const classHide = hide ? 'hide' : '';

    return <SimpleCard
      {...this.props}
      style={style}
      className={[className, classAnimate, classHide].join(' ')}
    />;
  }
}
